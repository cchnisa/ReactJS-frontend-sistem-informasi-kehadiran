...
// sections
import { UserListHead, UserListToolbar } from '../sections/@dashboard/presensi';
// mock
import USERLIST from '../_mock/presensi';
...
const TABLE_HEAD = [
  ...
  { id: 'jamMasuk', label: 'Jam Masuk', alignRight: false },
  { id: 'jamKeluar', label: 'Jam Keluar', alignRight: false },
...
function applySortFilter(array, comparator, query) {
  ...
  if (query) {
    return filter(array, (presensi) => presensi.name.toLowerCase().indexOf(query.toLowerCase()) !== -1);
  }
...
export default function PresensiPage() {
  ...
  const [jamMasuk, setMasuk] = useState('');
  const [jamKeluar, setKeluar] = useState('');
  ...
  const handleOpenDetail = (detail) => {
    ...
    setName(detail.name);
    setNip(detail.nip);
    setMasuk(detail.jamMasuk);
    setKeluar(detail.jamKeluar);
          ...
          <Modal open={open} onClose={handleCloseMenu}>
            <Box sx={style} component="form" noValidate autoComplete="on">
              <FormControl sx={{ minWidth: '80%' }}>
                ...
                <TextField id="tanggal" value={tanggal} label="Tanggal" onChange={(e) => setTanggal(e.target.value)} />
                <TextField
                  id="jamMasuk"
                  value={jamMasuk}
                  label="Jam Masuk"
              ...
              <Table>
                ...
                <TableBody>
                  {filteredUsers.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage).map((row, i) => {
                    const { id, name, nip, tanggal, jamMasuk, jamKeluar } = row;
                    ...
                    return (
                      <TableRow hover key={id}>
                        ...
                        <TableCell align="left">{jamMasuk}</TableCell>

                        <TableCell align="left">{jamKeluar}</TableCell>

                        <TableCell align="left">
                          <Button onClick={() => handleOpenDetail(row)}>Detail</Button>
                        </TableCell>
                      </TableRow>
                    );
                  })}
